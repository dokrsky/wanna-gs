...
const third = { id: "third", role: "customer", displayName: "세 번째 합성 고객" };
...
function world(changes = {}) {
  ...
  let state = createInitialState({ products: [{ id: "p", name: "모의 상품" }], stores: [{ id: "s", name: "모의 점포" }], actors: [customer, other, third, merchant], conditions: [condition] }, { sessionId: "wait-check", generation: 1, now });
  let serial = 0;
...
{
  // UI08-P2: a later finalized pool can fulfill a still-linked request after
  // the larger FIFO head expires. Another unconfirmed order must not mask it.
  const w = world(); w.budget(); w.request("a", 3); w.request("b", 1, other);
  w.exec({ type: "order.approve", items: [{ productId: "p", quantity: 4, conditionVersion: "v1" }] });
  w.request("c", 2, third);
  w.exec({ type: "order.approve", items: [{ productId: "p", quantity: 2, conditionVersion: "v1" }] });
  w.exec({ type: "supply.finalize", lineId: w.state.lines[1].id, quantity: 2 });
  for (const r of w.state.requests.filter(r => r.id !== "a")) { r.consentAt += 1000; r.consentExpiresAt += 1000; }
  w.read("SUPPLY_CONFIRMATION_PENDING", now, "b", other);
  assert.ok(w.state.links.some(l => l.requestId === "b" && l.active));
  w.read("ALLOCATION_PENDING", now + P.consentMs, "b", other);
  assert.equal(w.state.reservations.length, 0, "reading must not reserve or pay");
  w.exec({ type: "clock.advance", milliseconds: P.consentMs });
  assert.equal(w.read(null, now, "b", other).reservation.status, "confirmed");
  assert.equal(w.state.lines[0].suppliedQuantity, null, "no extra supply needed");
}
console.log(`PASS waiting pure checker: ${checks} projections; 8 codes, terminal/reconsent/pool/active-order priority, private line/budget/aggregate projection, normal trade/pickup, entire state and viewer unchanged. No SQL/browser/build/live.`);
