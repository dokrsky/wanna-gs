...
| UI-06 | Meitner: 정책 AI API. Newton: 정책 변경안/확인 UI. 조정자 통합 | API/화면 handback·자체 검사 완료, 별도 커밋/build/Preview·live 확인 중 |
| UI-07 | Meitner: 제한 대화 API. Locke: domain DTO/기록 명령/SQL migration 단일 작성. 후속 고객 UI 연결 | ADR-005 두 독립 검토 후 채택·DTO ACK, 구현 중. 기록만으로 거래 실행하지 않으며 기존 사본 보존 |
